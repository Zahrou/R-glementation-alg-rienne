# -*- coding: utf-8 -*-
"""
Created on Sun Oct 14 23:04:11 2018
...
years = get_urllist()[1]
chdir("D:/Desktop/Journal officiel2/2006")
...
def get_string(fichier):
    ...
    with open (fichier, "rb") as file:
        ...
        for i in range(conte.getNumPages()):
            try:
                if i == 0:
                    ...
                    page1 = page1.replace("”", "é").replace("’", "ê").replace("‘", "è").replace("Õ", "'").replace(to_strip, '').replace(to_strip_two, "").replace("ž", "û").replace("Ž", "ô").replace("septebre", "septembre")
                    page1 = page1.replace("  ", " ")
                    date = re.search(r'(Dimanche|Samedi|Lundi|Mardi|Mercredi|Jeudi|Vendredi)?(\w|\s)*(1|2)?([0-9]{1})?(\w|\s)*1[0-9]{3}(\s)*(correspondant|Correspondant)(\w|\s)*(janvier|février|mars|avril|mai|juin|juillet|août|aôut|aout|septembre|octobre|novembre|décembre)(\w|\s|\d)*(1|2)[0-9]{3}', page1, re.IGNORECASE).group(0)
                    date = date.lower()
                    date = date.replace("  ", " ")
                    if "au" not in date:
                        date_hijri =  date.split("correspondant")[0].strip()
                        date_grigorian =  date.split("correspondant")[1].strip()
                    else:
        ...
        if "ŠŠŠŠ" in jo:
            sommaire = jo[:jo.index("ŠŠŠŠ")]
        elif "ÑÑÑÑ" in jo:
            sommaire = jo[:jo.index("ÑÑÑÑ")]
        elif "————" in jo:
            sommaire = jo[:jo.index("————")]
...
class journalOfficiel:
    ...
    def construir_journal(self):
        ...
        if len(self.journal) > 100:
            ...
            self.sommaire = self.sommaire.replace("”", "é").replace("’", "ê").replace("‘", "è").replace("Õ", "'").replace("ž", "û").replace("›", "â").replace("Ž", "ô")     
            ...
            #Deleting any duplicate occurences
            for text in self.texts:
                if len(self.texts[text]) > 1:
                    for e in self.texts[text]:
                        templist = [e.replace(" ", "") for e in self.texts[text]]
                        if templist[0][:100] == templist[-1][:100]:
                            self.texts[text] = self.texts[text][:-1]
                            break
